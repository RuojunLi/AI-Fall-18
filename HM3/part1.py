...
def compute_L(a,y):
    ...
    ## INSERT YOUR CODE HERE
    if a ==1:
        L = -y*math.log(a)+(1-y)*1e6
    elif a ==0:
        L = -(1-y)*math.log(1-a)+y*1e6
    else:
...
def compute_dL_dw(dL_da, da_dz, dz_dw):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_dw =  np.multiply(dz_dw,dL_da*da_dz)
...
We repeat n_epoch passes over all the training instances.
    ...
    for _ in range(n_epoch):
        for x,y in zip(X,Y):
            ...
            w = update_w(w,compute_dL_dw(dL_da,da_dz,dz_dw),alpha)
            b = update_b(b,compute_dL_db(dL_da,da_dz,dz_db),alpha)
